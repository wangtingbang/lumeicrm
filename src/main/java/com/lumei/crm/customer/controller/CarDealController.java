...
public class CarDealController {
	...
	public ModelAndView list2() {
		...
		SessionUtil.setAttributes("dateStart", 
				DateTimeUtil.fromDate(
						DateTimeUtil.plusDays(DateTimeUtil.plusHours(DateTimeUtil.now(), -15), -1),
						DateTimeUtil.Pattern.DEFAULT_FORMATE_DATE));
		SessionUtil.setAttributes("dateEnd", 
				DateTimeUtil.fromDate(
						DateTimeUtil.plusDays(DateTimeUtil.plusHours(DateTimeUtil.now(), -15), -1),
	...
	public String export(CarDealQueryParam carDeal, int timezoneOffset, Model model, HttpServletResponse response) throws UnsupportedEncodingException {
	    ...
	    String filename = "Car_Deals_Export.xls";
	...
	private Pagination<CarDeal> process(CarDealQueryParam carDeal, int timezoneOffset, int page, int limit){
		...
		if (null != carDeal.getDealDateStart()) {
			carDeal.setDealDateStart(
					DateTimeUtil.plusMinutes(carDeal.getDealDateStart(), (serverOffset + timezoneOffset)));
		...
		if (null != carDeal.getModifiedDateStart()) {
			carDeal.setModifiedDateStart(
					DateTimeUtil.plusMinutes(carDeal.getModifiedDateStart(), (serverOffset + timezoneOffset)));
			param.put("modifiedDateStart", carDeal.getModifiedDateStart());
		}
		if (null != carDeal.getModifiedDateEnd()) {
			carDeal.setModifiedDateEnd(
					DateTimeUtil.plusMinutes(carDeal.getModifiedDateEnd(), (serverOffset + timezoneOffset)));
	...
	public String save(CarDeal carDeal, int timezoneOffset) {
		...
		if ("0".equals(carDeal.getId())) {
			...
			String caseNo = LumeiCrmConstants.CaseNo.CarSale + String.format("%06d", record2.getCurrentValue());
